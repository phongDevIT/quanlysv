...
const FormAttendence = ({ editId, open, setOpen, refetch }: Props) => {
  const [form] = useForm();
  ...
  const { data } = useQuery(
    ["attendence"],
    () => attendenceService.getAttendenceById(editId as number),
    {
  ...
  const createMutation = useMutation({
    ...
    mutationFn: (body: {
      ...
      subject_id: number;
      number: string;
    }) => attendenceService.newAttendence(body),
  ...
  const updateMutation = useMutation({
    ...
    mutationFn: (body: {
      ...
      subject_id: number;
      number: string;
    }) => attendenceService.updateAttendence(editId as number, body),
    onSuccess(data, _variables, _context) {
      const res = data.data;
      if (!res) return;
    ...
    >
      ...
      >
        ...
        <Form.Item label="Tên lớp" name="class_id">
          <Select options={listClassByDepartment} onChange={handleChangeClass}/>
        </Form.Item>
        ...
        <Form.Item label="Tên môn học" name="subject_id">
        ...
        <Form.Item
          label="Buổi"
          name="number"
          rules={[{ required: true, message: "Vui lòng ký buổi" }]}
        >
          <Input type="number" />
        </Form.Item>
